...
public class Export {
    ...
    public static boolean exportToExcel(List<Transaction> transactions, String exportPath, CoinPrice coinPrices, String fiatCurrency, Locale localeDecimal,String exportSplitter) {
        try {
            PrintWriter writer = new PrintWriter(new File(exportPath));
            StringBuilder sb = new StringBuilder();

            sb.append("Date,Operation,Amount,Cryptocurrency,FIAT value,FIAT currency,Block Hash,Block Height".replace(",",exportSplitter)+"\n");
            ...
            for (int iTransaction = 0; iTransaction < transactions.size(); iTransaction++) {
                ...
                for (int i = 0; i < transactions.get(iTransaction).amounts.length; i++) {
                    ...
                    sb.append(TransactionMethods.convertTimeStampToString(transactions.get(iTransaction).blockTime) + exportSplitter);
